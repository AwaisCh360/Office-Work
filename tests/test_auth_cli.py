"""Tests for the `apex auth` CLI: subcommand routing and provider naming."""
...
from apex.interface import auth_cli
...
def _tmp_store(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(codex, "AUTH_PATH", tmp_path / "home" / ".apex" / "subscription-auth.json")
