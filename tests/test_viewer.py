"""Tests for the local run viewer (apex.interface.viewer) and its path helpers."""
...
from apex.core.paths import latest_run_dir, runs_base_dir
from apex.interface.viewer.server import serve
from apex.interface.viewer.transcript import (
    build_run_state,
...
def test_latest_run_dir_none_when_no_runs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert runs_base_dir() == tmp_path / "apex_runs"
...
def test_server_serves_api_and_static(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("apex.interface.viewer.server.bundle_dir", lambda: assets)
...
def test_server_event_endpoint_forwards_cta(
    ...
    monkeypatch.setattr(
        "apex.telemetry.posthog.viewer_cta_clicked",
...
def test_server_event_endpoint_forwards_email_funnel(
    ...
    seen: list[tuple[str, str | None]] = []
    monkeypatch.setattr(
        "apex.telemetry.posthog.viewer_email_event",
...
def test_server_event_endpoint_forwards_agent_steered(
    ...
    seen: list[bool] = []
    monkeypatch.setattr("apex.telemetry.posthog.viewer_agent_steered", lambda: seen.append(True))
...
def test_feedback_records_telemetry_on_success(
    ...
    sent: list[bool] = []
    monkeypatch.setattr("apex.interface.viewer.auth.feedback_submit", lambda *_a: None)
    monkeypatch.setattr(
        "apex.telemetry.posthog.viewer_feedback_submitted", lambda: sent.append(True)
    )
...
def _cookie_name(url: str) -> str:
    ...
    return f"apex_viewer_session_{urlsplit(url).port}"
...
def test_auth_status_reflects_expiry(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _bundle(tmp_path, monkeypatch)
    monkeypatch.setattr(
        "apex.interface.viewer.auth.read_auth", lambda: {"email": "a@b.com", "token": "t"}
    )
    verified = {"value": True}
    monkeypatch.setattr("apex.interface.viewer.auth.is_verified", lambda: verified["value"])
...
def test_auth_mutations_require_session(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("apex.interface.viewer.auth.forget", lambda: forgotten.update(value=True))
...
def test_runs_list_requires_session_and_verification(
    ...
    monkeypatch.setattr("apex.interface.viewer.auth.is_verified", lambda: True)
