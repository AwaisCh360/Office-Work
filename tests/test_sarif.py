"""Tests for the SARIF 2.1.0 emitter in apex.report.sarif."""
...
def test_write_sarif_never_embeds_poc_script(tmp_path: Path) -> None:
    ...
    poc = _read(tmp_path)["runs"][0]["results"][0]["properties"]["apex"]["poc"]
...
def test_write_sarif_emits_version_control_provenance(tmp_path: Path) -> None:
    ...
    assert run["automationDetails"] == {"id": "apex/acme/widget"}
