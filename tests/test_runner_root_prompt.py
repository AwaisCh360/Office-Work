"""Tests for root scan prompt options in run_apex_scan.

Verify that ``root_instructions_override`` and ``extra_system_prompt_context``
flow through to the root agent's ``build_apex_agent`` call.
"""
...
import apex.tools.notes.tools as notes_tools
import apex.tools.todo.tools as todo_tools
from apex.core import runner
...
def _patch_engine_scaffold(
    ...
    """Stub out everything around build_apex_agent and stop at run_agent_loop.

    Returns a dict that will be populated with the kwargs the runner passed to
    ``build_apex_agent`` for the root agent.
    """
    ...
    def _build_apex_agent(**kwargs: Any) -> object:
        if kwargs.get("is_root") and "kwargs" not in captured:
            captured["kwargs"] = kwargs
        return object()

    monkeypatch.setattr(runner, "build_apex_agent", _build_apex_agent)
