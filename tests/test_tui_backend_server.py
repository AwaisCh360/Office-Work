...
from apex.interface.tui.backend.projection import terminal_projection
from apex.interface.tui.backend.protocol import (
    MAX_COMMAND_BYTES,
...
from apex.interface.tui.live_view import TuiLiveView
...
async def test_collection_bootstrap_is_chunked_deltas_are_incremental_and_idle_is_silent() -> None:
    ...
    try:
        ...
        while not (complete == {"agents", "events", "vulnerabilities"} and state_received):
            size, message = await asyncio.wait_for(receive_frame(child), timeout=15)
            if message["type"] == "state":
                state_received = True
