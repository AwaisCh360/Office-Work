...
from apex.config import ContextSettings
from apex.llm import compaction
...
def _patch_summary(
    ...
    monkeypatch.setattr(compaction, "ApexProvider", FakeProvider)
...
async def test_maybe_compact_skips_when_summary_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(compaction, "ApexProvider", BoomProvider)
    session = FakeSession(_turns(12))
    before = await session.get_items()
