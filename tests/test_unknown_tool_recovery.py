...
Models regularly invent tool names that Apex does not register (``read_file``
is a common one, borrowed from other agent frameworks). The SDK's default is to
raise ``ModelBehaviorError``, which ends the whole run: nothing in Apex retries
...
from apex.config.models import _NonStreamingModel
...
def _agent() -> Agent[Any]:
    ...
    return Agent(name="Apex", instructions="test", tools=[real_tool], model="gw-model")
