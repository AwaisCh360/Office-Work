...
if [ -n "${APEX_HOST_UID:-}" ] && [ "${APEX_HOST_UID}" != "0" ] && [ "${APEX_HOST_UID}" != "$(id -u)" ]; then
  exec sudo -E -- bash -c '
    set -e
    gid="${APEX_HOST_GID:-$APEX_HOST_UID}"
    old_uid="$1"
    old_gid="$2"
    export PATH="$3"
    shift 3
    sed -i "s|^pentester:x:${old_uid}:${old_gid}:|pentester:x:${APEX_HOST_UID}:${gid}:|" /etc/passwd
    sed -i "s|^pentester:x:${old_gid}:|pentester:x:${gid}:|" /etc/group
    chown -R "${APEX_HOST_UID}:${gid}" /home/pentester /app/certs
    chown "${APEX_HOST_UID}:${gid}" /workspace
    exec setpriv --reuid "${APEX_HOST_UID}" --regid "${gid}" --init-groups "$0" "$@"
  ' "$0" "$(id -u)" "$(id -g)" "$PATH" "$@"
fi
...
# hostname (rather than an IP literal), set APEX_CAIDO_ALLOWED_DOMAINS to a
...
if [ -n "${APEX_CAIDO_ALLOWED_DOMAINS:-}" ]; then
  IFS=',' read -ra _caido_domains <<< "${APEX_CAIDO_ALLOWED_DOMAINS}"
