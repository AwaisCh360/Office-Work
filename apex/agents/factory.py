"""Build SandboxAgents for root + child Apex runs."""
...
from apex.tools.agents_graph.tools import (
    agent_finish,
    create_agent,
...
from apex.tools.finish.tool import finish_scan
from apex.tools.load_skill.tool import load_skill
from apex.tools.notes.tools import (
    create_note,
    delete_note,
    get_note,
    list_notes,
...
from apex.tools.output_store import bound_and_store, bound_text
from apex.tools.proxy.tools import (
    list_requests,
    list_sitemap,
    repeat_request,
    scope_rules,
...
from apex.tools.reporting.tool import (
    create_dependency_report,
    create_vulnerability_report,
    get_report,
    list_reports,
)
from apex.tools.respond.tool import respond_to_user
from apex.tools.thinking.tool import think
from apex.tools.todo.tools import (
    create_todo,
    delete_todo,
    list_todos,
    mark_todo_done,
...
from apex.tools.web_search.tool import web_search
...
def _with_bounded_result(tool: FunctionTool) -> FunctionTool:
    ...
    if getattr(tool, "_apex_bounded", False):
        return tool
    ...
    tool._apex_bounded = True  # type: ignore[attr-defined]
    return tool
...
def _with_coerced_arguments(tool: FunctionTool) -> FunctionTool:
    if getattr(tool, "_apex_coerced", False):
        return tool
    invoke_tool = tool.on_invoke_tool
    ...
    tool.on_invoke_tool = invoke
    tool._apex_coerced = True  # type: ignore[attr-defined]
    return tool
...
# ``apex.runtime.backends.register_backend``: register before the first
# ``build_apex_agent`` call and every agent (root + children) gets them.
