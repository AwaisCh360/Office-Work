...
- **Apex config:** `containers/Dockerfile` sets `AGENT_BROWSER_*` env
  (executable path, UA, launch args, screenshot dir).
- **Skill:** `apex/skills/tooling/agent_browser.md` — **always-loaded**
  into every agent prompt by `apex/agents/prompt.py:_resolve_skills`.
