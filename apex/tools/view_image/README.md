...
- **Implementation:** `agents.sandbox.capabilities.tools.view_image.ViewImageTool`
  (upstream `agents` SDK)
- **Wired in:** `apex/agents/factory.py` — added per-run via the SDK
  `Filesystem` capability.
- **Apex defaults:** screenshots default to
...
- **Skill:** screenshot workflow lives in `apex/skills/tooling/agent_browser.md`.
- **Recovery:** vision-not-supported model rejections are auto-recovered
  via `apex.core.sessions.strip_all_images_from_session`, invoked from
  `apex/core/execution.py`.
