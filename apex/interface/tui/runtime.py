...
from apex.config import load_settings, persist_current
from apex.core.agents import AgentCoordinator
from apex.core.hooks import BudgetExceededError
from apex.core.runner import run_apex_scan
...
from apex.interface.tui.backend import TuiBackendServer, TuiController
from apex.interface.tui.backend.live_view import TuiLiveView
from apex.interface.tui.sidecar import (
    check_return_code,
    child_environment,
    launch_tui_process,
    package_version,
...
from apex.report.state import ReportState, set_global_report_state
from apex.utils.resource_paths import get_apex_resource_path
...
class GoTuiRuntime:
    ...
    async def _run_scan(self) -> None:
        image = str(load_settings().runtime.image or "apex-sandbox:latest")
        try:
            await run_apex_scan(
    ...
    def binary_command() -> list[str]:
        ...
        if (source / "go.mod").is_file() and shutil.which("go"):
            return ["go", "run", "./cmd/apex-tui"]
        packaged = get_apex_resource_path("bin", tui_executable())
        if packaged.is_file():
            return [str(packaged)]
        raise RuntimeError(
            "Bubble Tea TUI binary not found. Reinstall Apex from an official platform wheel."
        )
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            env["APEX_VERSION"] = package_version()
