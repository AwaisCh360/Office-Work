...
func (m Model) setupSummaryView(width int) string {
	menuBadge := lipgloss.NewStyle().Bold(true).Background(green).Foreground(black).Render(" ⚙️ Menu: TAB ")
	chips := []string{menuBadge}
	...
	chips = append(chips, render.Col(brightGreen).Render("mode:"+scanMode))

	scopeMode := m.snapshot.ScopeMode
	if scopeMode == "" {
		scopeMode = "auto"
	}
	chips = append(chips, render.Col(lightBlue).Render("scope:"+scopeMode))

	if m.snapshot.MaxBudgetUSD != nil {
		chips = append(chips, render.Dim().Render(fmt.Sprintf("$%.2f budget", *m.snapshot.MaxBudgetUSD)))
	} else {
		chips = append(chips, render.Dim().Render("no budget limit"))
	}
	if m.snapshot.MaxTurns > 0 {
		chips = append(chips, render.Dim().Render(fmt.Sprintf("%d turns", m.snapshot.MaxTurns)))
	}
...
func (m Model) setupHintsView(width int) string {
	...
	left := hint("enter", "launch") + label("  ") + hint("tab", "scan config menu") + label("  ") + hint("ctrl+c", "quit")
