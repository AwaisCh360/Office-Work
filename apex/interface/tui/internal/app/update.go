...
func (m Model) updateMain(key tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "f2", "ctrl+e", "ctrl+p", "ctrl+k":
		if m.snapshot.SetupMode {
			m.openModal(modalSetupConfig)
			return m, nil
		}
	case "f3":
		if m.snapshot.SetupMode {
			return m.cycleSetupConfigRow(1, 1)
		}
	case "f4":
		if m.snapshot.SetupMode {
			return m.cycleSetupConfigRow(2, 1)
		}
	case "f5":
		if m.snapshot.SetupMode {
			return m.cycleSetupConfigRow(3, 1)
		}
	case "tab":
		if m.snapshot.SetupMode {
			m.openModal(modalSetupConfig)
...
func (m Model) updateModal(key tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	if m.modal == modalSetupConfig {
		switch key.String() {
		case "esc":
			m.closeModal()
			return m, nil
		case "up", "k":
			m.setupMenuRow = (m.setupMenuRow + 3) % 4
			return m, nil
		case "down", "j":
			m.setupMenuRow = (m.setupMenuRow + 1) % 4
			return m, nil
		case "left", "h":
			return m.cycleSetupConfigRow(m.setupMenuRow, -1)
...
func (m Model) cycleSetupConfigRow(row, delta int) (tea.Model, tea.Cmd) {
	switch row {
	case 0:
		// Scan Mode
		modes := []string{"quick", "standard", "deep"}
		curr := m.snapshot.ScanMode
		if curr == "" {
			curr = "deep"
		}
		idx := 0
		...
		next := modes[(idx+delta+len(modes))%len(modes)]
		return m, send(m.client, "setup.set_scan_mode", map[string]any{"scan_mode": next})
	case 1:
		// Scope Mode
		scopes := []string{"auto", "diff", "full"}
		curr := m.snapshot.ScopeMode
		if curr == "" {
			curr = "auto"
		}
		idx := 0
		...
		next := scopes[(idx+delta+len(scopes))%len(scopes)]
		return m, send(m.client, "setup.set_scope_mode", map[string]any{"scope_mode": next})
	case 2:
		// Max Budget
		budgets := []any{10.0, 25.0, 50.0, nil}
		idx := 3
		if m.snapshot.MaxBudgetUSD != nil {
			val := *m.snapshot.MaxBudgetUSD
			if val <= 15.0 {
				idx = 0
			} else if val <= 35.0 {
				idx = 1
		...
		next := budgets[(idx+delta+len(budgets))%len(budgets)]
		return m, send(m.client, "setup.set_max_budget", map[string]any{"max_budget_usd": next})
	case 3:
		// Max Turns
		turns := []int{100, 300, 500, 1000}
		curr := m.snapshot.MaxTurns
		if curr <= 0 {
			curr = 500
		}
		idx := 2
		...
		next := turns[(idx+delta+len(turns))%len(turns)]
		return m, send(m.client, "setup.set_max_turns", map[string]any{"max_turns": next})
	}
	return m, nil
}
