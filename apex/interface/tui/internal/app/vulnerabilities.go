...
func (m Model) modalView() string {
	...
	case modalSetupConfig:
		return m.setupConfigModalView()
...
func (m Model) setupConfigModalView() string {
	width := min(58, max(36, m.width-4))
	inner := lipgloss.NewStyle().Width(width - 4)

	scanMode := m.snapshot.ScanMode
	if scanMode == "" {
		scanMode = "deep"
	}
	scopeMode := m.snapshot.ScopeMode
	if scopeMode == "" {
		scopeMode = "auto"
	}
	maxTurns := m.snapshot.MaxTurns
	if maxTurns <= 0 {
		maxTurns = 500
	}

	renderOption := func(label string, active bool) string {
		if active {
			return lipgloss.NewStyle().Bold(true).Background(green).Foreground(black).Render(" " + label + " ")
		}
		return lipgloss.NewStyle().Foreground(textColor).Render(" " + label + " ")
	}

	rowLabel := func(idx int, name string) string {
		if m.setupMenuRow == idx {
			return render.Bold(brightGreen).Render("▸ " + name)
		}
		return render.Dim().Render("  " + name)
	}

	// Row 0: Scan Mode
	r0 := rowLabel(0, "Scan Depth: ") +
		renderOption("Quick", scanMode == "quick") + " " +
		renderOption("Standard", scanMode == "standard") + " " +
		renderOption("Deep", scanMode == "deep")

	// Row 1: Scope Mode
	r1 := rowLabel(1, "Scope Mode: ") +
		renderOption("Auto", scopeMode == "auto") + " " +
		renderOption("Diff", scopeMode == "diff") + " " +
		renderOption("Full", scopeMode == "full")

	// Row 2: Max Budget
	b10 := m.snapshot.MaxBudgetUSD != nil && *m.snapshot.MaxBudgetUSD == 10.0
	b25 := m.snapshot.MaxBudgetUSD != nil && *m.snapshot.MaxBudgetUSD == 25.0
	b50 := m.snapshot.MaxBudgetUSD != nil && *m.snapshot.MaxBudgetUSD == 50.0
	bNo := m.snapshot.MaxBudgetUSD == nil
	r2 := rowLabel(2, "Max Budget: ") +
		renderOption("$10", b10) + " " +
		renderOption("$25", b25) + " " +
		renderOption("$50", b50) + " " +
		renderOption("Unlimited", bNo)
	...
	t500 := maxTurns == 500
	t1000 := maxTurns == 1000
	r3 := rowLabel(3, "Max Turns: ") +
		renderOption("100", t100) + " " +
		renderOption("300", t300) + " " +
		renderOption("500", t500) + " " +
		renderOption("1000", t1000)

	title := inner.Bold(true).Foreground(brightGreen).Align(lipgloss.Center).Render("⚙️ APEX SCAN CONFIGURATION MENU")
	body := r0 + "\n\n" + r1 + "\n\n" + r2 + "\n\n" + r3
	hints := render.Dim().Render("[↑/↓ Navigate]  [←/→/Space Cycle]  [Esc Done]")

	content := title + "\n\n" + body + "\n\n" + inner.Align(lipgloss.Center).Render(hints)
	return lipgloss.NewStyle().Width(width-2).Border(lipgloss.RoundedBorder()).
		BorderForeground(green).Background(black).Padding(1, 2).Render(content)
}
