"""UI-independent state and command controller for interactive Apex clients."""
...
from apex.config.models import is_recommended_or_frontier_model
from apex.config.settings import DEFAULT_MAX_TURNS
from apex.interface.tui.backend.live_view import TuiLiveView
from apex.interface.tui.backend.projection import (
    MAX_TERMINAL_EVENTS,
...
from apex.interface.utils import is_subscription_run


if TYPE_CHECKING:
    import argparse

    from apex.report.state import ReportState
...
class TuiController:
    ...
    async def handle(self, command: str, payload: dict[str, Any]) -> dict[str, Any]:
        handlers = {
            ...
            "setup.set_scan_mode": self._set_scan_mode,
            "setup.set_scope_mode": self._set_scope_mode,
            "setup.set_max_budget": self._set_max_budget,
    ...
    async def _set_scan_mode(self, payload: dict[str, Any]) -> dict[str, Any]:
        self._require_setup_mutable()
        scan_mode = self._required_string(payload, "scan_mode")
        if scan_mode not in SCAN_MODES:
            raise ValueError(f"Invalid scan mode: {scan_mode}")
        self.scan_mode = scan_mode
        self.args.scan_mode = scan_mode
        return {"scan_mode": self.scan_mode}

    async def _set_scope_mode(self, payload: dict[str, Any]) -> dict[str, Any]:
        self._require_setup_mutable()
        scope_mode = self._required_string(payload, "scope_mode")
        if scope_mode not in SCOPE_MODES:
            raise ValueError(f"Invalid scope mode: {scope_mode}")
        self.scope_mode = scope_mode
        self.args.scope_mode = scope_mode
        diff_base = payload.get("diff_base")
        if isinstance(diff_base, str):
            self.diff_base = diff_base.strip() or None
            self.args.diff_base = self.diff_base
        return {"scope_mode": self.scope_mode, "diff_base": self.diff_base}

    async def _set_max_budget(self, payload: dict[str, Any]) -> dict[str, Any]:
        self._require_setup_mutable()
        raw_budget = payload.get("max_budget_usd")
        if raw_budget is None:
            self.max_budget_usd = None
            self.args.max_budget_usd = None
        else:
            try:
                ...
                self.args.max_budget_usd = budget
            except (ValueError, TypeError):
                raise ValueError("max_budget_usd must be a positive number") from None
        return {"max_budget_usd": self.max_budget_usd}

    async def _set_max_turns(self, payload: dict[str, Any]) -> dict[str, Any]:
        self._require_setup_mutable()
        raw_turns = payload.get("max_turns")
        try:
            turns = int(raw_turns)
            if turns <= 0:
                raise ValueError
            self.max_turns = turns
            self.args.max_turns = turns
        except (ValueError, TypeError):
    ...
    async def _start(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        model = (load_settings().llm.model or "").strip()
        if not model:
            raise ValueError("No model configured. Set APEX_LLM first.")
    ...
    async def _begin_scan(self, verify: bool) -> None:
        ...
        try:
            ...
            self.scan_state = "failed"
            self.error = str(exc)
            logger.exception("Failed to start scan: %s", exc)
            raise
        finally:
            self._start_in_progress = False
            self.notify_changed()
    ...
    async def _open_viewer(self, _payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            from apex.interface.tui.backend.messages import (
                send_user_message_to_agent,
            )
            from apex.interface.viewer.server import (
                authorized_url,
                bundle_is_built,
                serve,
            )
            ...
            with contextlib.suppress(Exception):
                from apex.telemetry import posthog
