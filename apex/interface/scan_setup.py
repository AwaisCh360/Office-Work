...
telemetry live here so ``apex.interface.main`` (the CLI) and
``apex.interface.tui.runtime`` (interactive setup) depend on one module
...
from apex.config import Settings, codex, load_settings
...
from apex.utils.api_spec import (
    SpecParseError,
...
async def preflight_model_connection(
    ...
    *,
    ...
    from apex.config.models import ApexProvider, configure_sdk_model_defaults
    ...
    model = ApexProvider().get_model(model_name)
...
def _persist_run_record(args: argparse.Namespace) -> None:
    from apex.report.writer import write_run_record
