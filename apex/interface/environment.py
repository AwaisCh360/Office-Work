...
def validate_environment() -> None:
    ...
    if codex.subscription_model(settings.llm.model):
        if not codex.is_authenticated():
            console.print(
                f"[red]APEX_LLM={settings.llm.model} uses your ChatGPT subscription, "
                "but you're not signed in.[/] Run [cyan]apex auth login chatgpt[/] first."
            )
            sys.exit(1)
    ...
    if not settings.llm.model:
        missing_required_vars.append("APEX_LLM")
    ...
    if missing_required_vars:
        ...
        for var in missing_required_vars:
            if var == "APEX_LLM":
                ...
                error_text.append("APEX_LLM", style="bold cyan")
                error_text.append(
        ...
        if missing_optional_vars:
            ...
            for var in missing_optional_vars:
                ...
                elif var == "APEX_REASONING_EFFORT":
                    error_text.append("• ", style="white")
                    error_text.append("APEX_REASONING_EFFORT", style="bold cyan")
                    error_text.append(
        ...
        error_text.append("\nExample setup:\n", style="white")
        error_text.append("export APEX_LLM='openai/gpt-5.4'\n", style="dim white")
        ...
        if missing_optional_vars:
            for var in missing_optional_vars:
                if var == "LLM_API_BASE":
                    ...
                    error_text.append(
                        "export APEX_REASONING_EFFORT='high'\n",
                        style="dim white",
                    )
...
def pull_docker_image() -> None:
    ...
    console.print()
    console.print(f"[dim]Pulling sandbox image[/] {image}")
    console.print("[dim yellow]This only happens on first run and may take a few minutes...[/]")
    console.print()

    # If native docker CLI is available, run directly for parallel bulk downloading & rich layer progress
    if shutil.which("docker"):
        try:
            ...
            logger.debug("Subprocess docker pull failed, falling back to python SDK", exc_info=True)
