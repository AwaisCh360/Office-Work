"""`apex auth` — ChatGPT subscription sign-in (login / status / logout).

Signing in only stores OAuth tokens (``~/.apex/subscription-auth.json``); model
selection stays with ``APEX_LLM``. A ``chatgpt/<model>`` APEX_LLM runs on the
subscription.
"""
...
from apex.config import codex, load_settings
...
def run_auth(argv: list[str]) -> int:
    """Entry point for ``apex auth …``. Returns a process exit code."""
    console = Console()
    # Bare `apex auth` (no subcommand) defaults to login.
...
def _login(console: Console, argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="apex auth login", add_help=True)
    parser.add_argument(
        "provider",
        nargs="?",
...
def _status(console: Console) -> int:
    ...
    if record is None:
        console.print("[yellow]Not signed in.[/] Run [cyan]apex auth login chatgpt[/] to sign in.")
        return 1
    ...
    if codex.subscription_model(settings.llm.model):
        console.print(f"  Runs use the subscription (APEX_LLM=[bold]{settings.llm.model}[/]).")
    else:
        console.print(
            "  [yellow]Note:[/] set [cyan]APEX_LLM[/] to e.g. [cyan]chatgpt/gpt-5.4[/] "
...
def _print_success(console: Console) -> None:
    ...
    text.append("APEX_LLM", style="bold white")
    ...
    text.append("apex --target https://example.com", style="bold cyan")
    console.print()
    console.print(
        Panel(
            text,
...
def _logo_img_tag() -> str:
    """Return an ``<img>`` for the Apex logo as an inline data URI, or "".
