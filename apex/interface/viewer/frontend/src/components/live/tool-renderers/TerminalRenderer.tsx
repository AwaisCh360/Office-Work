...
// TUI strips them in apex/interface/tui/renderers/shell_renderer.py). Only a
...
function cleanOutput(raw: string, command: string = ""): string {
  ...
  if (cleaned.trim()) {
    ...
    for (const line of lines) {
      ...
      // Skip [APEX_N]$ prompt lines
      if (/^\[APEX_\d+\]\$\s*/.test(line)) continue;
    ...
    // Strip trailing [APEX_N]$ lines
    while (filtered.length > 0 && /^\[APEX_\d+\]\$\s*/.test(filtered[filtered.length - 1])) {
      filtered.pop();
    }
