...
/**
 * Pure, dependency-free parsers that turn a Apex CLI local run
 * (`apex_runs/<run>/{run.json,vulnerabilities.json}`) into the app's own
...
function parseJson(text: string, label: string): unknown {
  try {
    ...
    throw new RunParseError(
      `${label} isn't valid JSON. Make sure you selected a Apex run directory.`
    );
  }
}
