...
from apex.core.paths import run_record_path
from apex.interface.viewer import auth
...
# port is appended (``apex_viewer_session_<port>``) because browsers scope
...
SESSION_COOKIE_PREFIX = "apex_viewer_session"
...
class _ViewerState:
    def __init__(
        ...
        # The apex_runs directory that holds the launched run; used to
        ...
        # standalone ``apex view`` / finished runs, so steering is unavailable.
...
def _make_handler(state: _ViewerState) -> type[BaseHTTPRequestHandler]:
    class ViewerHandler(BaseHTTPRequestHandler):
        ...
        def _handle_report_send(self) -> None:
            ...
            from apex.interface.viewer.report_pdf import build_encrypted_report
...
def serve(
    ...
    *,
    ...
    use, falls back to an ephemeral port. Reused by both the ``apex view``
    ...
    ``None`` (standalone ``apex view``), steering is reported unavailable.
    """
    ...
    thread = threading.Thread(target=httpd.serve_forever, name="apex-viewer", daemon=True)
    thread.start()
