...
the Apex relay (``APEX_APP_URL``). The browser never talks to the relay
directly, and the report password generated locally is never sent to it.

State lives in ``~/.apex/viewer-auth.json`` (0600). ``is_verified`` is a local
...
AUTH_PATH = Path.home() / ".apex" / "viewer-auth.json"
...
def feedback_submit(email: str, message: str) -> None:
    """Relay a feedback message + email to Apex. No verification is required;
