"""Command-line argument parsing for the ``apex`` scan entrypoint."""
...
from apex.config import apply_config_override
from apex.config.settings import DEFAULT_MAX_TURNS
from apex.core.paths import run_dir_for, runtime_state_dir
from apex.interface.scan_setup import attach_workspace_mount, build_targets_info
from apex.interface.update_check import self_update
from apex.interface.utils import (
    check_mountable_dir,
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apex Multi-Agent Cybersecurity Penetration Testing Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Web application penetration test
  apex --target https://example.com

  # GitHub repository analysis
  apex --target https://github.com/user/repo
  apex --target git@github.com:user/repo.git

  # Local code analysis
  apex --target ./my-project

  # API spec test (OpenAPI/Swagger file or Postman collection export)
  apex --target ./openapi.yaml --target https://api.example.com
  apex --target ./collection.postman_collection.json

  # Postman collection pulled live by id (needs POSTMAN_API_KEY); optional environment
  apex --target postman://<collection-uuid> --target https://api.example.com
  ...
  apex --target 192.168.1.42

  # Multiple targets (e.g., white-box testing with source and deployed app)
  apex --target https://github.com/user/repo --target https://example.com
  apex --target ./my-project --target https://staging.example.com --target https://prod.example.com

  # Targets from a file, one target per non-empty, non-comment line
  apex --target-list ./targets.txt

  # Custom instructions (inline)
  apex --target example.com --instruction "Focus on authentication vulnerabilities"

  # Custom instructions (from file)
  apex --target example.com --instruction-file ./instructions.txt
  apex --target https://app.com --instruction-file /path/to/detailed_instructions.md
        """,
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"apex {get_version()}",
    )

    parser.add_argument(
        "--update",
        action="store_true",
        help="Update apex to the latest version and exit. Self-updates the "
    ...
    parser.add_argument(
        "--config",
        type=str,
        help="Path to a custom config file (JSON) to use instead of ~/.apex/cli-config.json",
    )
    ...
    parser.add_argument(
        ...
        metavar="RUN_NAME",
        help=(
            "Resume a prior scan by its run name (the dir under ./apex_runs/). "
...
def _load_resume_state(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    from apex.report.writer import read_run_record
