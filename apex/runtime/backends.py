"""Sandbox backend registry — selected via APEX_RUNTIME_BACKEND (default: docker)."""
...
async def _docker_backend(
    *,
    ...
    Uses :class:`ApexDockerSandboxClient` to inject NET_ADMIN /
    ...
    Apex manages session lifetime explicitly via ``client.delete()`` so we
    trigger ``start()`` ourselves.
    """
    import docker
    from agents.sandbox.sandboxes.docker import DockerSandboxClientOptions

    from apex.runtime.docker_client import ApexDockerSandboxClient

    client = ApexDockerSandboxClient(docker.from_env())
    client.apex_bind_mounts = bind_mounts or []
...
def get_backend(name: str) -> SandboxBackend:
    ...
    if backend is None:
        ...
        raise ValueError(
            f"Unknown APEX_RUNTIME_BACKEND: {name!r} (supported: {supported})",
        )
