...
from apex.agents.factory import build_apex_agent, make_child_factory
from apex.agents.prompt import render_system_prompt
...
from apex.core.agents import AgentCoordinator
from apex.core.execution import (
    respawn_subagents,
    run_agent_loop,
)
from apex.core.execution import (
...
from apex.core.hooks import BudgetExceededError, ReportUsageHooks, recomputed_budget_flags
from apex.core.inputs import (
    build_root_task,
    build_scope_context,
    make_model_settings,
)
from apex.core.paths import run_dir_for, runtime_state_dir
from apex.core.sessions import open_agent_session
from apex.report.state import get_global_report_state
from apex.runtime import session_manager
from apex.telemetry.logging import set_scan_id, setup_scan_logging
from apex.tools.output_store import (
    WORKSPACE_SPILL_DIR,
...
if TYPE_CHECKING:
    ...
    from apex.runtime.status import StatusSink
...
async def run_apex_scan(
    *,
    ...
    """Run or resume one Apex scan against a sandbox.
    ...
    logger.info(
        "%s Apex scan %s (image=%s, max_turns=%d, interactive=%s, run_dir=%s)",
    ...
    if not resolved_model:
        raise RuntimeError(
            "No LLM model configured. Set APEX_LLM env or pass model= to run_apex_scan().",
        )
    ...
    from apex.tools.notes.tools import hydrate_notes_from_disk
    from apex.tools.todo.tools import hydrate_todos_from_disk
    ...
    try:
        ...
        logger.warning(
            ...
            "Resume with 'apex --resume %s' once the limit clears.",
            scan_id,
            exc,
            scan_id,
        )
        ...
        logger.exception("Apex scan %s failed", scan_id)
        if root_id is not None:
            await coordinator.cancel_descendants(root_id)
            with contextlib.suppress(Exception):
        ...
        logger.info("Apex scan %s done", scan_id)
        teardown_logging()
