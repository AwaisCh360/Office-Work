...
```bash
mkdir -p /workspace/.apex-source-aware
```
...
art = Path("/workspace/.apex-source-aware")
...
semgrep scan --config p/default --config p/golang --config p/secrets \
  --metrics=off --json --output /workspace/.apex-source-aware/semgrep.json .

# If you choose auto config, do not combine it with --metrics=off
semgrep scan --config auto --json --output /workspace/.apex-source-aware/semgrep-auto.json .
```
...
xargs -r -n 200 sg run --pattern '$F($$$ARGS)' --json=stream \
  < /workspace/.apex-source-aware/sg-targets.txt \
  > /workspace/.apex-source-aware/ast-grep.json 2> /workspace/.apex-source-aware/ast-grep.log || true
```
...
```bash
gitleaks detect --source . --report-format json --report-path /workspace/.apex-source-aware/gitleaks.json
trufflehog filesystem --json . > /workspace/.apex-source-aware/trufflehog.json
```
...
```bash
trivy fs --scanners vuln,misconfig --timeout 30m --offline-scan \
  --format json --output /workspace/.apex-source-aware/trivy-fs.json . || true
```
...
```bash
retire --path . --outputformat json --outputpath /workspace/.apex-source-aware/retire.json || true
eslint --no-config-lookup --rule '{"no-eval":2,"no-implied-eval":2}' \
  -f json -o /workspace/.apex-source-aware/eslint.json . || true
```
