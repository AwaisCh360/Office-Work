...
from apex.config import codex
from apex.config.loader import load_settings
from apex.config.tool_call_ids import TurnCallIdRewriter, dedupe_input
from apex.config.tool_call_limits import TurnToolCallLimiter
...
if TYPE_CHECKING:
    ...
    from apex.config.settings import LlmSettings, ReasoningEffort, Settings
...
class _NonStreamingModel(Model):
    ...
    Apex uses only issues streamed requests, so such a gateway fails every
...
class ApexProvider(MultiProvider):
...
def configure_sdk_model_defaults(settings: Settings) -> None:
    """Apply Apex config to SDK-native defaults."""
    llm = settings.llm
...
def _configure_litellm_compatibility() -> None:
    ...
    # Apex uses LiteLLM's success callback to capture provider-reported cost.
...
def _install_openrouter_stream_cost_capture() -> None:
    ...
    lost and Apex falls back to a cost-map estimate that is missing entirely
    ...
    from apex.report.state import streamed_openrouter_costs

    class _ApexOpenRouterStreamingHandler(OpenRouterChatCompletionStreamingHandler):
    ...
    class _ApexOpenrouterConfig(OpenrouterConfig):
        def get_model_response_iterator(
            self, streaming_response: Any, sync_stream: bool, json_mode: bool | None = False
        ) -> Any:
            return _ApexOpenRouterStreamingHandler(
    ...
    litellm.OpenrouterConfig = _ApexOpenrouterConfig  # type: ignore[misc]
...
def _configure_extra_headers(llm: LlmSettings) -> None:
    ...
    regardless of the ``APEX_LLM`` prefix.
    """
...
def _register_litellm_cost_callback() -> None:
    ...
    from apex.report.state import litellm_cost_callback
