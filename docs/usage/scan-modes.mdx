...
Apex offers three scan modes to balance speed and thoroughness.

## Quick

```bash
apex --target ./app --scan-mode quick
```
...
```bash
apex --target ./app --scan-mode standard
```
...
```bash
apex --target ./app --scan-mode deep
```
