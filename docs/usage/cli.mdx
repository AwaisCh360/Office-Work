---
title: "CLI Reference"
description: "Command-line options for Apex"
---

## Basic Usage

```bash
apex (--target <target> | --target-list <path>) [options]
```
...
<ParamField path="--target, -t" type="string">
  ...
  When the target is an API spec, Apex copies it into the agent's workspace and authorizes the base URLs it declares (including those resolved from a Postman environment) as in-scope hosts - so the agent reads the contract and tests the full declared surface instead of discovering endpoints by crawling. Pair the spec with the deployed base URL (e.g. `--target ./openapi.yaml --target https://api.example.com`) so the agent has a reachable host to attack.
...
<ParamField path="--config" type="string">
  Path to a custom config file (JSON) to use instead of `~/.apex/cli-config.json`.
</ParamField>
...
<ParamField path="--max-budget" type="number">
  ...
  **Limitations**
  ...
  - For LiteLLM-routed models, Apex enables streaming success callbacks to
...
apex --target https://app.com --instruction "Use credentials: user:pass"

# Focused testing
apex --target api.example.com --instruction "Focus on IDOR and auth bypass"

# CI/CD mode
apex -n --target ./ --scan-mode quick

# Cap cost and per-agent turns
apex --target https://example.com --max-budget 25 --max-turns 300

# Force diff-scope against a specific base ref
apex -n --target ./ --scan-mode quick --scope-mode diff --diff-base origin/main

# Multi-target white-box testing
apex -t https://github.com/org/app -t https://staging.example.com

# API spec + live target (OpenAPI/Swagger file or Postman collection)
apex -t ./openapi.yaml -t https://api.example.com

# Postman collection pulled live by id (+ optional environment)
apex -t "postman://<collection-uuid>?env=<environment-uuid>"
