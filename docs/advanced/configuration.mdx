---
title: "Configuration"
description: "Environment variables for Apex"
---

Configure Apex using environment variables or a config file.

## LLM Configuration

<ParamField path="APEX_LLM" type="string" required>
...
<ParamField path="APEX_LLM_MAX_RETRIES" default="5" type="integer">
  Maximum number of retries for LLM API calls on transient failures.
</ParamField>

<ParamField path="APEX_REASONING_EFFORT" default="high" type="string">
...
<ParamField path="APEX_MEMORY_COMPRESSOR_TIMEOUT" default="30" type="integer">
...
<ParamField path="APEX_DEDUPE_MODEL" type="string">
  Model used to judge whether a candidate finding duplicates an existing report.
  Falls back to `APEX_LLM` when unset.
</ParamField>
...
<ParamField path="APEX_DEDUPE_REASONING_EFFORT" type="string">
...
<ParamField path="APEX_TELEMETRY" default="1" type="string">
...
<ParamField path="TRACELOOP_BASE_URL" type="string">
  OTLP/Traceloop base URL for remote OpenTelemetry export. If unset, Apex keeps traces local only.
</ParamField>
...
When remote OTEL vars are not set, Apex still writes complete run telemetry locally to:

```bash
apex_runs/<run_name>/events.jsonl
```

When remote vars are set, Apex dual-writes telemetry to both local JSONL and the remote OTEL endpoint.

## Docker Configuration

<ParamField path="APEX_IMAGE" default="ghcr.io/AwaisCh360/office-work-sandbox:0.2" type="string">
...
<ParamField path="APEX_RUNTIME_BACKEND" default="docker" type="string">
  Runtime backend for the sandbox environment.
</ParamField>

## Sandbox Configuration

<ParamField path="APEX_SANDBOX_EXECUTION_TIMEOUT" default="120" type="integer">
  Maximum execution time in seconds for sandbox operations.
</ParamField>

<ParamField path="APEX_SANDBOX_CONNECT_TIMEOUT" default="10" type="integer">
  Timeout in seconds for connecting to the sandbox container.
</ParamField>

## Config File

Apex stores configuration in `~/.apex/cli-config.json`. You can also specify a custom config file:

```bash
apex --target ./app --config /path/to/config.json
```
...
{
  "env": {
    "APEX_LLM": "openai/gpt-5.4",
    "LLM_API_KEY": "sk-...",
    "APEX_REASONING_EFFORT": "high"
  }
}
```
...
export APEX_SANDBOX_EXECUTION_TIMEOUT="300"
