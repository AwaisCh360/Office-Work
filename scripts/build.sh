...
echo -e "${BLUE}🦉 Apex Build Script${NC}"
echo "================================"
...
TUI_BINARY="build/sidecar/apex-tui"
...
uv run pyinstaller apex.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="apex-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/apex.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/apex.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    cp "dist/apex" "$RELEASE_DIR/$BINARY_NAME"
