...
TUI_BINARY := build/sidecar/apex-tui$(if $(filter Windows_NT,$(OS)),.exe)
...
security:
	...
	uv run bandit -r apex/ -c pyproject.toml
...
viewer:
	...
	cd apex/interface/viewer/frontend && npm ci && npm run build
	@echo "✅ Viewer built to apex/interface/viewer/static/ (commit the changes)."
...
tui-build:
	...
	cd apex/interface/tui && CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o ../../../$(TUI_BINARY) ./cmd/apex-tui

tui-test:
	cd apex/interface/tui && go test -race ./...

tui-lint:
	cd apex/interface/tui && test -z "$$(gofmt -l .)" && go vet ./...
